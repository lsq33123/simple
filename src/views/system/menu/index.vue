  ...
  <div class="app-container">
    ...
    <el-table v-loading="loading" :data="menuList" row-key="menuId" :tree-props="{ children: 'children', hasChildren: 'hasChildren' }">
      ...
      <el-table-column prop="perms" label="权限标识" :show-overflow-tooltip="true">
        <template slot-scope="scope">
          {{scope.row.menuType == 4 ? scope.row.apiPath : scope.row.perms}}
        </template>
      </el-table-column>
...
<script>
import { getMenuList, addMenuList,delMenuList,updateMenuList } from "@/api/system/menu";
  ...
  methods: {
    ...
    getList() {
      ...
      getMenuList(this.queryParams).then(response => {
        this.menuList = this.handleTree(response.result.list, "menuId");
        this.loading = false;
      });
    },
    /** 转换菜单数据结构 */
    ...
    getTreeselect() {
      getMenuList().then(response => {
        ...
        menu.children = this.handleTree(response.result.list, "menuId");
      ...
      this.$refs["form"].validate(valid => {
        if (valid) {
          let params = {
              ...
              sort: this.form.orderNum,
              url_path: this.form.path,
              api_path: this.form.apiPath,
              component: this.form.component,
              is_frame: this.form.isFrame,
              is_cache: this.form.isCache,
              type: this.form.menuType,
          ...
          if (this.form.menuId != undefined) {
            updateMenuList(params).then(response => {
              this.msgSuccess("修改成功");
              this.open = false;
              this.getList();
            });
          } else {
            addMenuList(params).then(response => {
    ...
    handleDelete(row) {
      this.$confirm('是否确认删除名称为"' + row.menuName + '"的数据项?', "警告", {
        ...
        .then(function() {
          return delMenuList({id:row.menuId});
        })
        .then(() => {
          this.getList();
